#!/bin/bash
aws cloudformation list-exports --query 'Exports[?Name==`brave-sls-toaster-jtw-security-'"$STAGE"'`].Value' --output text > rolearn.txt
aws cloudformation list-exports --query 'Exports[?Name==`brave-sls-toaster-jtw-security-secrets-'"$STAGE"'`].Value' --output text > s3.txt
